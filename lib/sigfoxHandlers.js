...
function generatePayload(queryParams, device, callback) {
    ...
    function createPayload(data, callback) {
        config.getLogger().debug('Here received data from decoder of type: ', typeof data);
        config.getLogger().debug('Here received data from decoder: ', data);
        if(!data instanceof Array){
            config.getLogger().debug('Here not an array');
            data = new Array(data);
        }
        config.getLogger().debug('Here processed data: ', data);
