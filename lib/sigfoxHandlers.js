...
function getFields(req) {
    var fields = req.method == 'POST' ? req.body : req.query;
    if (!fields.id) {
        fields.id = fields.device;
    }
    return fields;
}
...
function requiredFields(req, res, next) {
    ...
    if (!fields.id || !fields.data) {
        var notFoundParams = [];

        if (!fields.id) {
            notFoundParams.push('id/device');
        }
...
function generatePayload(queryParams, device, callback) {
    ...
    function createPayload(data, callback) {
        ...
        for (var i in queryParams) {
            if (queryParams.hasOwnProperty(i) && i !== 'data' && i !== 'id' && i != 'device') {
                attributes.push({
                    name: i,
                    type: 'String',
                    value: queryParams[i]
...
function handleMeasure(req, res, next) {
    const fields = getFields(req);
    config.getLogger().debug(context, 'Handling request with query [%j]', fields);
...
function loadContextRoutes(router) {
    ...
    router.post('/sww/update', requiredFields, handleMeasure);
}
